...
export default function EscrowCustomerDetail() {
  ...
  const uploadMutation = useMutation({
    mutationFn: async ({ name, dataUrl }: { name: string; dataUrl: string }) => {
      const res = await apiRequest("POST", `/api/escrow/my-transactions/${id}/documents`, { name, dataUrl });
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/escrow/my-transactions', id] });
